...
class SCLiteAlignment:
    ...
    def get_full_report (self, oracle, variant):
        ...
        try:
            report = {"Corrects":int(score_line[-4]),"Substitutions":int(score_line[-3]),"Deletions":int(score_line[-2]),"Insertions":int(score_line[-1]), "Length":int(len(ref.split(" ")))}
        except:
            report = {"Corrects":0,"Substitutions":0,"Deletions":0,"Insertions":0, "Length":0}
    ...
    def get_confusions(self, hyp, ref):
        ...
        _, eval_report = self.get_full_report(ref, hyp)
